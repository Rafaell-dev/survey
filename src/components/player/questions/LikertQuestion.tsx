      ...
      <div className="flex flex-col sm:flex-row justify-between items-stretch sm:items-end min-w-[300px] gap-2">
        {question.scaleOptions.map((option) => (
          ...
          >
            ...
            <div className={`w-6 h-6 sm:w-8 sm:h-8 rounded-full border-2 flex items-center justify-center transition-all ${value === option.id ? 'border-primary bg-primary' : 'border-input group-hover:border-primary/50'}`}>
              {value === option.id && <div className="w-2.5 h-2.5 sm:w-3 sm:h-3 bg-white rounded-full" />}
            </div>
          </div>
        ))}
      </div>
